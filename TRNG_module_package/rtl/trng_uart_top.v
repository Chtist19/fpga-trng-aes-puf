...
module trng_uart_top (
    ...
    localparam integer TAIL_BYTES = 4;

    localparam integer FRAME_BYTES = 2 + 2 + RAW_BYTES + HASH_BYTES + TAIL_BYTES;
            ...
            case (state)
                ...
                ST_TX_LOAD: begin
                    if (!uart_busy) begin
                        ...
                        end else if (tx_index < 8'd164) begin
                            uart_data <= hash_mem[tx_index - 8'd132];
                        end else if (tx_index == 8'd164) begin
                            uart_data <= 8'h5A;
                        end else if (tx_index == 8'd165) begin
                            uart_data <= 8'hA5;
                        end else if (tx_index == 8'd166) begin
                            uart_data <= 8'h33;
                        end else begin
                            uart_data <= 8'hCC;
                        end
